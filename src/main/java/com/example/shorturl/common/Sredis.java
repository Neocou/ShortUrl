...
public class Sredis {
    ...
    private void createJedisPool() {
        ...
        log.info("create jedis pool:"+host+" "+port+" "+password);
    }
    ...
    public Long getAndIncr(String key){
        ...
        try {
            redisClient = getJedis();
            return redisClient.incrBy(prefix+"::"+key,1);
        } catch (Exception e) {
            log.error("incrby string error:"+e.getMessage());
        }  finally { // 还原到连接池
            returnRes(redisClient);
        }
        return null;
